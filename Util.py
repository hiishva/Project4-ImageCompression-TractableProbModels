...
def load_dataset(filename):
    data = np.loadtxt(filename, delimiter=',', dtype=np.uint32)
    smoothedData = data + 1
    return smoothedData
'''
...
def compute_weighted_xycounts(dataset,weights):
    nvariables=dataset.shape[1]
    prob_xy = np.zeros((nvariables, nvariables, 2, 2))
    prob_xy[:, :, 0, 0] = np.einsum('ij,ik->jk', (dataset == 1).astype(int)* weights[:, np.newaxis], (dataset == 1).astype(int))
    prob_xy[:, :, 0, 1] = np.einsum('ij,ik->jk', (dataset == 1).astype(int)* weights[:, np.newaxis], (dataset == 2).astype(int))
    prob_xy[:, :, 1, 0] = np.einsum('ij,ik->jk', (dataset == 2).astype(int)* weights[:, np.newaxis], (dataset == 1).astype(int))
    prob_xy[:, :, 1, 1] = np.einsum('ij,ik->jk', (dataset == 2).astype(int)* weights[:, np.newaxis], (dataset == 2).astype(int))
    return prob_xy
...
def compute_weighted_xcounts(dataset,weights):
    nvariables = dataset.shape[1]
    prob_x = np.zeros((nvariables, 2))
    prob_x[:,0]=np.einsum('ij->j',(dataset == 1).astype(int) * weights[:, np.newaxis])
    prob_x[:,1] = np.einsum('ij->j',(dataset == 2).astype(int) * weights[:, np.newaxis])
    return prob_x
'''
